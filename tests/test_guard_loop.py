...
def runner(
    *,
    ...
    is the loop, and the loop reaches exactly these five attributes.
    """
...
def test_a_tick_reconnects_before_reading() -> None:
    ...
    jarvis, broker, _, _ = runner(positions=[object()])
    jarvis.guard_tick()
...
def test_events_are_recorded() -> None:
    jarvis, _, manager, recorded = runner(positions=[object()])
...
def test_the_gap_between_cycles_is_spent_watching() -> None:
    jarvis, _, manager, _ = runner(guard_interval=0.01, positions=[object()])
...
def test_it_returns_at_the_deadline() -> None:
    ...
    jarvis, _, _, _ = runner(guard_interval=0.01, positions=[object()])
...
def test_zero_disables_the_guard_but_still_waits() -> None:
    ...
    jarvis, _, manager, _ = runner(guard_interval=0.0, positions=[object()])
    started = time.monotonic()
    jarvis._guard_until(started + 0.05)
...
def test_the_kill_switch_ends_the_wait_immediately() -> None:
    ...
    jarvis, _, manager, _ = runner(guard_interval=0.01, engaged=True, positions=[object()])
    started = time.monotonic()
...
def test_the_guard_never_sleeps_past_the_deadline() -> None:
    ...
    jarvis, _, _, _ = runner(guard_interval=5.0, positions=[object()])
    started = time.monotonic()
    jarvis._guard_until(started + 0.05)
    assert time.monotonic() - started < 1.0
