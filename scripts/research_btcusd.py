...
def main() -> int:
    ...
    row = connection.execute(
        "SELECT broker_symbol, spec_json FROM instruments " "WHERE canonical_symbol='BTCUSD'"
    ).fetchone()
    if row is None:
    ...
    for mechanism, timeframe, polarity, stop_atr, target_r, session in cells:
        ...
        label = f"{mechanism}/{timeframe}/{direction}/{stop_atr:g}ATR/" f"{target_r:g}R/{session}"
        train_results.append(summarise(label, "train", split_trades(trades, frame, 0.0, 0.5)))
    ...
    for wanted_timeframe in TIMEFRAMES:
        ...
        trades = trades.loc[clock_filter(trades, session)].copy()
        validation = summarise(winner.cell, "validation", split_trades(trades, frame, 0.5, 0.75))
        holdout = summarise(winner.cell, "holdout", split_trades(trades, frame, 0.75, 1.01))
        ...
        if managed:
            ...
            holdout = summarise(train.cell, "holdout", split_trades(candidate, frame, 0.75, 1.01))
