...
def load_bars(connection: sqlite3.Connection, symbol: str, timeframe: str) -> pd.DataFrame:
    ...
    frame = pd.DataFrame(rows, columns=["time", "open", "high", "low", "close", "volume", "spread"])
...
def main() -> int:
    ...
    points = {
        ...
        for symbol, spec in connection.execute("SELECT broker_symbol, spec_json FROM instruments")
    }
    ...
    for market in COMPONENTS:
        ...
        for (
            ...
            if (
                ...
                stable.append((floor, discovery_train, discovery_validation, discovery_holdout))
